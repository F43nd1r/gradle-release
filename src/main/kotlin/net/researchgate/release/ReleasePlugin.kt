...
class ReleasePlugin : PluginHelper(), Plugin<Project> {
    ...
    override fun apply(project: Project) {
        ...
        val service = project.gradle.sharedServices.registerIfAbsent("release", ReleaseService::class.java) {
        ...
        val p = project.path.let { if (!it.endsWith(Project.PATH_SEPARATOR)) it + Project.PATH_SEPARATOR else it }
        project.tasks.register("release", GradleBuild::class.java) {
            ...
            it.tasks = listOf("${p}createScmAdapter",
                    "${p}initScmAdapter",
                    "${p}checkCommitNeeded",
                    "${p}checkUpdateNeeded",
    ...
    protected fun findScmAdapter(): BaseScmAdapter {
        ...
        return ScmAdapterHolder.scmAdapter ?: extension.scmAdapters.map {
        ...
        }.firstOrNull { it.isSupported(projectPath) }?.also { ScmAdapterHolder.scmAdapter = it } ?: throw GradleException(
